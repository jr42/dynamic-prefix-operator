...
Dynamic Prefix Operator is a Kubernetes operator that manages dynamic IPv6 prefix delegation for bare-metal and home/SOHO clusters. It automatically updates Cilium pool resources (LoadBalancerIPPools, CIDRGroups) when ISP-delegated IPv6 prefixes change via DHCPv6-PD or Router Advertisements.
...
```bash
go test -v ./internal/prefix/subnet_test.go ./internal/prefix/subnet.go ./internal/prefix/types.go
```

Run tests matching a pattern:
```bash
go test -v ./... -run TestSubnetName
```

## Key Technologies

- **Go 1.24.6** with controller-runtime v0.22.4 (Kubebuilder 4.10.1)
- **insomniacslk/dhcp**: DHCPv6 Prefix Delegation client
- **mdlayler/ndp**: Router Advertisement (NDP) monitoring
